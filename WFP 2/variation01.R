...
#fip_mean = fip_mean[fip_mean$Year>2022,]

df <- fip_mean %>%
  group_by(NAME_2) %>%
  arrange(Year) %>%
  mutate(Variation = FIP - lag(FIP))

# fip_merg0=summary_stats <- df %>%
#   group_by(NAME_2) %>%
#   summarize(
#     Mean_Variation = mean(Variation, na.rm = TRUE),
#     Max_Variation = max(Variation, na.rm = TRUE),
#     Min_Variation = min(Variation, na.rm = TRUE),
#     Coef_Regression = coef(lm(FIP ~ Year))[2],
#     P_Value = summary(lm(FIP ~ Year))$coefficients[2, 4],
#     FIP = mean(FIP, na.rm = T)
#   )
# 
# df %>%
#   group_by(NAME_2) %>%
...
#   group_by(NAME_2) %>%
#   summarize(
#     Variation_M = mean(Variation, na.rm = TRUE),
#     FIP_M = mean(FIP, na.rm = TRUE),
#     CV_Variation = (sd(Variation, na.rm = TRUE) / mean(Variation, na.rm = TRUE)) * 100, 
#     CV_FIP = (sd(FIP, na.rm = TRUE) / mean(FIP, na.rm = TRUE)) * 100  
#   )
...
fip_merg <- merge(merged_shapefile, fip_merg0, by=c("NAME_2"))
...
map <- tm_shape(fip_merg) +
  tm_fill(col = "FIP", 
          title = "Mean of Food Insecure Population (%)", 
          style = "cont", 
          breaks = seq(0, 80, by = 10),  # Set breaks from 0 to 30 with steps of 5
...
map01 <- tm_shape(fip_merg) +
  tm_fill(col = "Variation", 
          title = "Variation of FIP between 2020 and 2024 (%)", 
          style = "cont", 
          breaks = seq(-9, 9, by = 2), 
          palette = mako(10, direction = -1), 
          legend.show = TRUE) +
...
fpath <- 'D:/OneDrive - CGIAR/1-Scripts/WFP/WFP 2/Products/FIP/variation/'
tmap_save(map, dpi= 300,  width=11, height =8, units="in",
          filename=paste0(fpath, paste0("FIP_Mean_2020_2024",year,".png")))

tmap_save(map01, dpi= 300,  width=11, height =8, units="in",
          filename=paste0(fpath, paste0("FIP_variation-2020_and_2024.png")))

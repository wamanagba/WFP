...
map <- tm_shape(fip_merg) +
  tm_fill(col = "FIP", 
          ...
          style = "cont", 
          breaks = seq(0, 100, by = 10),  # Set breaks from 0 to 30 with steps of 5
...
fpath <- 'D:/OneDrive - CGIAR/1-Scripts/WFP/WFP 2/Products/FIP/Final_last3/'
...
# Vérifier si des valeurs manquantes existent et les traiter
fip_merg$FIP <- ifelse(is.na(fip_merg$FIP), -1, fip_merg$FIP)  # Assigner -1 pour les valeurs manquantes
...
map <- tm_shape(fip_merg) +
  tm_fill(col = "FIP", 
          title = "Food Insecure Population (%)", 
          style = "fixed", 
          breaks = c(-1, 0, 20, 30, 40, 50, 60, 70, 80, 90, 100),  # Breaks incluant les valeurs manquantes
          palette = c("grey", "white", mako(8, direction = -1)),  # Grey pour les manquants, White pour <= 20%
          legend.show = TRUE, 
          labels = c("Missing", "≤ 20%", 
                     "30 - 40%", "40 - 50%", "50 - 60%", 

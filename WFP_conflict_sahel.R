...
reLabel <- function(conf){
  ...
  #county_conf$clust[i=="Moderate conflict + Moderate climate impact"] <- 7
...
tmap_mode("plot")
map <- tm_shape(conf) +
  tm_fill(col= "clust", palette= viridis(9,direction = -1, option = "H", alpha = 0.7), title="Conflict-climate intersection",
...
map <- tm_shape(conf) +
  ...
  tm_shape(Mali_adm0) +
  tm_borders(lty = "dashed", col = "red", lwd = 3) +
  tm_shape(Niger_adm0) +
  tm_borders(lty = "dashed", col = "grey", lwd = 4) +
  tm_add_legend(type = "line", col = "black", lwd = 1, title = "Regional Boundaries") +
  ...
  tm_add_legend(type = "line", lty = "dashed", col = "grey", lwd = 1.5, title = "Niger border") +
  tm_compass(type = "8star", position = c("right", "top")) +
  tm_scale_bar(breaks = c(0, 75, 150), text.size = 1, 
               position = c("right", "bottom"))+
  tm_mouse_coordinates()+

...
fip_mean$NAME_3[i=="HAUTS-BASSINS"] <- "HAUTS-BASSINS"
...
no_limited_label <- c("High conflict + High climate condition",
...
fpath <- '/Users/yacoub/Library/CloudStorage/OneDrive-CGIAR/SA_Team/Data/Burkina_Faso/Results/'
...
names(ICSMAG)[names(ICSMAG) == "adm2_name"] <- "NAME_1"
ICSMAG$NAME_1 <- toupper(ICSMAG$NAME_1)
nameCheck(BF_adm2, ICSMAG, level=2)

#merge icsmag and BF shapefile
merged_icsmag <- merge(BF_adm2, ICSMAG, by="NAME_1")
...
icsmag_map <- tm_shape(merged_icsmag)+
  ...
  tm_shape(BF_adm2)+
  tm_borders(col="black",lwd=0.01)+
  tm_text("NAME_1", size = 0.7, remove.overlap = TRUE, col ='gray70')+
  tm_compass(type = "8star", size=4,position = c("right", "bottom")) +
  tm_scale_bar(breaks = c(0, 50, 100), text.size = 1.5, 
               position = c("right", "bottom"))+
  tm_layout(legend.outside=F, 
